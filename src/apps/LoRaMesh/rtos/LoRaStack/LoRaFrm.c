        ...
        case FRM_TYPE_REGULAR:
            if ( fDir == DOWN_LINK ) {
                ...
                if ( fPort == 0 ) {
                    ...
                    LoRaMacPayloadEncrypt(
                            LORAFRM_BUF_PAYLOAD_START_WPORT(packet->phyData),
                            fPayloadSize, childNode->Connection.NwkSKey, devAddr, fDir,
                            fCnt, fBuffer);
                    ...
                    LoRaMacPayloadEncrypt(
                            LORAFRM_BUF_PAYLOAD_START_WPORT(packet->phyData),
                            fPayloadSize, childNode->Connection.AppSKey, devAddr, fDir,
                            fCnt, fBuffer);
                }
                // Decode frame options MAC commands
                if ( fCtrl.Bits.FOptsLen > 0 )
        ...
        case FRM_TYPE_MULTICAST:
            if ( fDir == DOWN_LINK ) {
                ...
                LoRaMacPayloadEncrypt(LORAFRM_BUF_PAYLOAD_START_WPORT(packet->phyData),
                        fPayloadSize, multicastGrp->Connection.AppSKey, devAddr, fDir,
                        fCnt, fBuffer);
            }
            break;
    }
