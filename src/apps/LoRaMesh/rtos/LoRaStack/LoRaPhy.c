...
static void OnRadioTxDone( void )
{
    ...
    if ( phyFlags.Bits.TxType == LORAPHY_TXTYPE_ADVERTISING ) {
        ...
        TimerSetValue(&RxWindowTimers[LORAPHY_RXSLOT_RX1WINDOW], pLoRaDevice->rxWindow1Delay);
        TimerStart(&RxWindowTimers[LORAPHY_RXSLOT_RX1WINDOW]);
        TimerSetValue(&RxWindowTimers[LORAPHY_RXSLOT_RX2WINDOW], pLoRaDevice->rxWindow2Delay);
