...
static IRQInterruptIndex g_uartIrq[] = { UART0_RX_TX_IRQn, UART1_RX_TX_IRQn,
        UART2_RX_TX_IRQn };
        ...
        FlowCtrl_t flowCtrl )
{
    ...
    if ( parity == NO_PARITY ) {
        ...
        if ( parity == EVEN_PARITY ) g_uartBase[obj->UartId]->C1 =
                (((g_uartBase[obj->UartId]->C1) & ~(UART_C1_PT_MASK)) | (UART_C1_PE_MASK));
    ...
    g_uartBase[obj->UartId]->BDH = (((g_uartBase[obj->UartId]->BDH)
            & (~(UART_BDH_SBR_MASK))) | (((uint8_t)(sbr >> 8)) & UART_BDH_SBR_MASK));
    g_uartBase[obj->UartId]->BDL = (((g_uartBase[obj->UartId]->BDL)
            & (~(UART_BDL_SBR_MASK))) | (((uint8_t) sbr) & UART_BDL_SBR_MASK));
    ...
    g_uartBase[obj->UartId]->C4 =
            (((g_uartBase[obj->UartId]->C4) & (~(UART_C4_BRFA_MASK)))
    ...
    if ( mode == TX_ONLY ) {
        ...
        g_uartBase[obj->UartId]->C2 |= (UART_C2_RIE_MASK | UART_C2_TE_MASK
                | UART_C2_RE_MASK); /* Enable transmitter, receiver & receiver interrupt */
    }

    /* Enable interrupt for UARTx */
    NVIC_BASE_PTR->ISER[(((uint32_t) (int32_t) g_uartIrq[obj->UartId]) >> 5UL)] =
            (uint32_t)(1UL << (((uint32_t) (int32_t) g_uartIrq[obj->UartId]) & 0x1FUL));
}
...
void UartMcuDeInit( Uart_t *obj )
{
    ...
    NVIC_BASE_PTR->ICER[(((uint32_t) (int32_t) g_uartIrq[obj->UartId]) >> 5UL)] =
            (uint32_t)(1UL << (((uint32_t) (int32_t) g_uartIrq[obj->UartId]) & 0x1FUL));
...
void UartInterruptHandler( Uart_t *obj )
{
    ...
    if ( (((g_uartBase[obj->UartId]->S1) & UART_S1_RDRF_MASK) >> UART_S1_RDRF_SHIFT)
            == 1 ) {
