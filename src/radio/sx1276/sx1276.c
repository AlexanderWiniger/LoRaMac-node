 ...
 */
#if defined(FSL_RTOS_FREE_RTOS) || defined(USE_FREE_RTOS)
void SX1276OnTimeoutIrq( void *param );
#else
 ...
 */
const FskBandwidth_t FskBandwidths[] = { { 2600, 0x17 }, { 3100, 0x0F }, { 3900, 0x07 }, {
        5200, 0x16 }, { 6300, 0x0E }, { 7800, 0x06 }, { 10400, 0x15 }, { 12500, 0x0D }, {
        15600, 0x05 }, { 20800, 0x14 }, { 25000, 0x0C }, { 31300, 0x04 }, { 41700, 0x13 },
        { 50000, 0x0B }, { 62500, 0x03 }, { 83333, 0x12 }, { 100000, 0x0A }, { 125000,
                0x02 }, { 166700, 0x11 }, { 200000, 0x09 }, { 250000, 0x01 }, { 300000,
 ...
 */
DioIrqHandler *DioIrq[] = { SX1276OnDio0Irq, SX1276OnDio1Irq, SX1276OnDio2Irq,
        SX1276OnDio3Irq, SX1276OnDio4Irq, NULL };
...
#if defined(FSL_RTOS_FREE_RTOS) || defined(USE_FREE_RTOS)
    TimerInit(&TxTimeoutTimer, "TxTimeoutTimer", LORAMESH_CONFIG_MAX_RX_WINDOW, TIMER_PRIORITY_LOW, SX1276OnTimeoutIrq, (void*) 0, false);
    TimerInit(&RxTimeoutTimer, "RxTimeoutTimer", LORAMESH_CONFIG_MAX_RX_WINDOW, TIMER_PRIORITY_LOW, SX1276OnTimeoutIrq, (void*) 0, false);
    TimerInit(&RxTimeoutSyncWord, "RxTimeoutSyncWord", 1e6, TIMER_PRIORITY_LOW, SX1276OnTimeoutIrq, (void*) 0, false);
#else
...
void SX1276SetRxConfig( RadioModems_t modem, uint32_t bandwidth, uint32_t datarate,
        ...
        uint16_t symbTimeout, bool fixLen, uint8_t payloadLen, bool crcOn, bool freqHopOn,
...
void SX1276SetTxConfig( RadioModems_t modem, int8_t power, uint32_t fdev,
        uint32_t bandwidth, uint32_t datarate, uint8_t coderate, uint16_t preambleLen,
        bool fixLen, bool crcOn, bool freqHopOn, uint8_t hopPeriod, bool iqInverted,
        uint32_t timeout )
{
    uint8_t paConfig = 0;
    uint8_t paDac = 0;
            ...
            double tmp = ceil(
                    (8 * pktLen - 4 * SX1276.Settings.LoRa.Datarate + 28
                            ...
                            / (double) (4 * SX1276.Settings.LoRa.Datarate
                                    - ((SX1276.Settings.LoRa.LowDatarateOptimize > 0) ?
                                            8 : 0)))
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    switch (SX1276.Settings.Modem) {
        case MODEM_FSK:
        {
            ...
            SX1276.Settings.FskPacketHandler.FifoThresh = SX1276Read(REG_FIFOTHRESH)
                    & 0x3F;
        ...
        case MODEM_LORA:
        {
            ...
            if ( SX1276.Settings.LoRa.Bandwidth < 9 ) {
                SX1276Write(REG_LR_DETECTOPTIMIZE,
                        SX1276Read(REG_LR_DETECTOPTIMIZE) & 0x7F);
                LOG_TRACE("Value at 0x%02x:\t 0x%02x.", REG_LR_DETECTOPTIMIZE,
                        SX1276Read(REG_LR_DETECTOPTIMIZE));
                ...
                SX1276Write(REG_LR_DETECTOPTIMIZE,
                        SX1276Read(REG_LR_DETECTOPTIMIZE) | 0x80);
                LOG_TRACE("Value at 0x%02x:\t 0x%02x.", REG_LR_DETECTOPTIMIZE,
                        SX1276Read(REG_LR_DETECTOPTIMIZE));
            }
...
void SX1276SetModem( RadioModems_t modem )
{
    ...
    switch (SX1276.Settings.Modem) {
        ...
        case MODEM_LORA:
            ...
            LOG_TRACE("Value at 0x%02x:\t 0x%02x.", REG_DIOMAPPING1,
                    SX1276Read(REG_DIOMAPPING1));
            LOG_TRACE("Value at 0x%02x:\t 0x%02x.", REG_DIOMAPPING2,
                    SX1276Read(REG_DIOMAPPING2));
            break;
    }
    LOG_TRACE("Leaving %s...", __FUNCTION__);
}
            ...
            switch (SX1276.Settings.Modem) {
                case MODEM_FSK:
                    ...
                    if ( (SX1276.Settings.FskPacketHandler.Size == 0)
                        ...
                        if ( SX1276.Settings.Fsk.FixLen == false ) {
                            SX1276ReadFifo(
                                    (uint8_t*) &SX1276.Settings.FskPacketHandler.Size, 1);
                        } else {
                            SX1276.Settings.FskPacketHandler.Size = SX1276Read(
                                    REG_PAYLOADLENGTH);
                        }
                        SX1276ReadFifo(
                                RxBuffer + SX1276.Settings.FskPacketHandler.NbBytes,
                ...
                case MODEM_LORA:
                {
                    ...
                    SX1276.Settings.LoRaPacketHandler.SnrValue = SX1276Read(
                            REG_LR_PKTSNRVALUE);
                    if ( SX1276.Settings.LoRaPacketHandler.SnrValue & 0x80 ) // The SNR sign bit is 1
                            {
                        // Invert and divide by 4
                        snr = ((~SX1276.Settings.LoRaPacketHandler.SnrValue + 1) & 0xFF)
                                >> 2;
                        snr = -snr;
                    } else {
                        // Divide by 4
...
#if defined(FSL_RTOS_FREE_RTOS) || defined(USE_FREE_RTOS)
                    SX1276ReadFifo(LORAPHY_BUF_PAYLOAD_START(RxBuffer), SX1276.Settings.LoRaPacketHandler.Size);
#else
            ...
            switch (SX1276.Settings.Modem) {
                case MODEM_FSK:
                    if ( (SX1276.Settings.FskPacketHandler.PreambleDetected == true)
                        ...
                        SX1276.Settings.FskPacketHandler.RssiValue = -(SX1276Read(
                                REG_RSSIVALUE) >> 1);

                        SX1276.Settings.FskPacketHandler.AfcValue =
                                (int32_t) (double) (((uint16_t) SX1276Read(REG_AFCMSB)
                                        << 8) | (uint16_t) SX1276Read(REG_AFCLSB))
                                        * (double) FREQ_STEP;
                        SX1276.Settings.FskPacketHandler.RxGain = (SX1276Read(REG_LNA)
                                >> 5) & 0x07;
                    }
                    break;
                case MODEM_LORA:
